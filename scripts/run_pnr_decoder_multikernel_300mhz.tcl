...
if {[file exists [file join $script_dir "kernel_HLS"]]} {
    set repo_root $script_dir
    set kernel_dir [file join $script_dir "kernel_HLS"]
} elseif {[file exists [file join $script_dir ".." "kernel_HLS"]]} {
    set repo_root [file normalize [file join $script_dir ".."]]
    set kernel_dir [file normalize [file join $script_dir ".." "kernel_HLS"]]
} else {
    set repo_root $script_dir
    set kernel_dir $script_dir
}

set output_dir [file join $repo_root "pnr_decoder_multikernel_300mhz_run"]
...
for {set pe 0} {$pe < 4} {incr pe} {
    ...
    set pe_vdir [file join $repo_root "proj_int4_decoder_pe${pe}" "solution_300mhz" "syn" "verilog"]
    if {![file exists $pe_vdir]} {
        set pe_vdir [file join $repo_root "proj_int4_decoder_pe${pe}" "solution_270mhz" "syn" "verilog"]
    }
...
set top_wrapper [file join $kernel_dir "int4_decoder_multikernel_top.v"]
if {![file exists $top_wrapper]} {
    set top_wrapper [file join $repo_root "int4_decoder_multikernel_top.v"]
}
if {![file exists $top_wrapper]} {
    set top_wrapper [file join $script_dir "int4_decoder_multikernel_top.v"]
}
if {![file exists $top_wrapper]} {
    error "ERROR: Top wrapper does not exist in $kernel_dir or $repo_root"
}
...
for {set pe 0} {$pe < 4} {incr pe} {
    set pe_sol_dir [file join $repo_root "proj_int4_decoder_pe${pe}" "solution_300mhz"]
    if {![file exists $pe_sol_dir]} {
        set pe_sol_dir [file join $repo_root "proj_int4_decoder_pe${pe}" "solution_270mhz"]
    }
