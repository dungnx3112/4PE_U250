...
entity int4_decoder_token_controller_model_norm_cache0_RAM_1P_URAM_2R1W is 
    generic(
        MEM_TYPE        : string    := "hls_ultra"; 
        DataWidth       : integer   := 512; 
        AddressWidth    : integer   := 13;
        AddressRange    : integer   := 4160
    ); 
    port (
...
architecture rtl of int4_decoder_token_controller_model_norm_cache0_RAM_1P_URAM_2R1W is 
...
begin 
    if (clk'event and clk = '1') then
        if (ce0 = '1') then 
            q0_t0 <= ram(CONV_INTEGER(address0_tmp));
            if (we0 = '1') then 
