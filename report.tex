...
\begin{tikzpicture}[x=1cm,y=1cm]
    \node[localbox, fill=u250orange] (qkv) at (0, 0) {BRAM\\\texttt{q, k, v}};
    \node[pebox, minimum width=2.5cm] (attn) at (4, 0) {SwiftKV Attention\\\& RoPE};
    \node[ddrbox] (rope) at (4, 2) {DDR\\RoPE LUT};
    \node[ddrbox] (kv) at (4, -2) {DDR\\KV Cache};
    \node[pebox, minimum width=2.5cm] (lin_o) at (8, 0) {Linear\\(O)};
    \node[reducebox, minimum width=2.5cm] (add) at (12, 0) {Residual\\Add};
    
    \draw[dataline] (qkv) -- (attn);
    \draw[dataline] (rope) -- (attn);
    \draw[bothline] (kv) -- (attn);
    \draw[dataline] (attn) -- node[above, font=\scriptsize] {\texttt{act\_q}} (lin_o);
    \draw[dataline] (lin_o) -- node[above, font=\scriptsize] {\texttt{projection}} (add);
\end{tikzpicture}
}
\caption{Luồng thực thi của khối Attention và chiếu Linear O.}
\end{figure}
...
Khối FFN trong Llama là sự kết hợp cực kỳ tối ưu về bộ nhớ nhờ thực thi tuần tự:
\begin{enumerate}
...
\begin{tikzpicture}[x=1cm,y=1cm]
    \node[pebox, minimum width=2cm] (rms) at (0, 0) {RMSNorm\\(FFN)};
    \node[pebox, minimum width=2.2cm] (gate) at (3.5, 1.5) {Linear\\(Gate)};
    \node[pebox, minimum width=2.2cm] (up) at (3.5, -1.5) {Linear\\(Up)};
    \node[localbox, fill=u250orange] (gate_mem) at (7, 1.5) {BRAM\\\texttt{gate}};
    \node[localbox, fill=u250orange] (proj_mem) at (7, -1.5) {BRAM\\\texttt{projection} (chứa Up)};
    \node[pebox, minimum width=2cm] (swiglu) at (11, 0) {SwiGLU\\\& Lượng tử};
    \node[pebox, minimum width=2.2cm] (down) at (14.5, 0) {Linear\\(Down)};
    \node[reducebox, minimum width=2cm] (add) at (18, 0) {Residual\\Add};
    
    \draw[dataline] (-2, 0) -- node[above, font=\scriptsize] {residual} (rms);
    \draw[dataline] (rms) -- ++(1,0) |- (gate);
    \draw[dataline] (rms) -- ++(1,0) |- (up);
    
    \draw[dataline] (gate) -- (gate_mem);
    \draw[dataline] (up) -- (proj_mem);
    
    \draw[dataline] (gate_mem) -| (swiglu);
    \draw[dataline] (proj_mem) -| (swiglu);
    
    \draw[dataline] (swiglu) -- node[above, font=\scriptsize] {\texttt{act\_q}} (down);
    \draw[dataline] (down) -- node[above, font=\scriptsize] {\texttt{projection}} (add);
    
    \draw[dataline] (-1, 0) -- ++(0,-3) -| (add);
    \node[font=\scriptsize, align=center] at (8.5, -3.2) {Đường truyền residual (skip connection)};
\end{tikzpicture}
}
\caption{Luồng thực thi tuần tự của khối FFN tiết kiệm bộ nhớ trên FPGA.}
\end{figure}
